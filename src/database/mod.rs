...
mod tests {
    ...
    fn claim_one_runnable_task() {
        ...
        assert_eq!(tasks_for_worker_1.unwrap().first().unwrap().data, Some("Subtask 2".to_string()));
        assert_eq!(tasks_for_worker_2.unwrap().first().unwrap().data, Some("Subtask 1".to_string()));
    }
}
